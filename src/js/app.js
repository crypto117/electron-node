...
$(document).ready(() => {
  ...
  const execFile = path.join(process.cwd(), ((os.platform() === 'win32') ? 'TurtleCoind.exe' : 'TurtleCoind'))
  const dataDir = path.join(getHomeDir(), ((os.platform() === 'win32') ? 'TurtleCoin' : '.TurtleCoin'))
  const startArgs = [
    '--rpc-bind-ip 0.0.0.0',
